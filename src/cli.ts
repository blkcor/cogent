...
// Check if we should enter REPL mode (no arguments provided)
const shouldEnterRepl = process.argv.length === 2

if (shouldEnterRepl) {
  // Enter REPL mode
  render(React.createElement(Repl))
  // Don't parse commands when in REPL
} else {
  ...
  program
    ...
    .action(async (taskArgs: string[], options) => {
      ...
      // Use TUI by default
      if (options.ui !== false) {
        const { waitUntilExit } = render(React.createElement(App, { task }))
        await waitUntilExit()
        return
      }
